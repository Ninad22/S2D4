...
m = 0
journalists_downcase = journalists.map {|emails| emails.downcase}
  journalists_downcase.each do |emails|
    if emails.match(/aude/)
      m += 1
    end
end
puts "#{m}"
...
puts journalists.sort_by {|emails| emails.downcase}


puts "Quels sont les 50 handles les plus courts de ce array ?"
journalists_shortlist = journalists.map.sort_by {|emails| emails.length}
puts journalists_shortlist.take(50)
